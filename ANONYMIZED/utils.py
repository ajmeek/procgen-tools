...
def cd_into_ANONYMIZED():
    ...
    while Path.cwd().name != "ANONYMIZED":
        if Path.cwd().parent == Path.cwd():  # we're at the root
            os.chdir(original_path)
            raise Exception("Could not find ANONYMIZED directory")
        os.chdir("..")
...
def setup_dir():
    ...
    try:
        ...
        Path("ANONYMIZED").mkdir(parents=True, exist_ok=True)
        os.chdir("ANONYMIZED")
...
def setup(
    ...
    """
    cd into the ANONYMIZED directory then download and extract data files.
    """
    setup_dir()
    assert Path.cwd().name == "ANONYMIZED", "must be in ANONYMIZED"
